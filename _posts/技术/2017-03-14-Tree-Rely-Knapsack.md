...
> 以下将针对树形依赖的背包问题和hdu1561的实际案例来对本文的核心问题进行思考。
...
而从泛化的角度来考虑，首先，如果是森林的话我们可以考虑添加超级根来将原问题转换成一棵树上的问题。

对于树上的问题：

我们将每个节点u及其子树作为一个泛化物品，而最后的答案就是根节点所代表的泛化物品中的信息。

设f[i][j]为给i的子树分配j的体积所能获得的最大收益，那么f[i]代表的就是i和它的子树形成的泛化物品。

对于每个叶子节点v所代表的泛化物品的结果，可以直接算出f[v][0]=0,f[v][1]=value[v]。

所以只需要考虑泛化物品的求和。

把两个泛化物品合并成一个泛化物品的运算，就是枚举体积分配给两个泛化物品，满足

<img src="http://latex.codecogs.com/gif.latex?G[i] = max(G_1[j] + G_2[i - j])" title="G[i] = max(G_1[j] + G_2[i - j])" />
...
for (int i = head[u]; i; i = e[i].next) {
        ...
        s = Calc(v, u);
        for (int j = 0; j <= C; j++)
            g[j] = f[u][j], f[u][j] = -0x7ffffff;
        for (int j = 0; j <= C; j++)
            for (int k = 0; k <= s; k++) {
                if (j + k >= m) break ;
                f[u][j + k] = max(f[v][k] + g[j], f[u][j + k]);
            }
        C += s; 
    } 
```

在以上代码中，C代表该泛化物品最大分配体积，g数组代表的是之前子树的泛化物品之和，即对于每个儿子，将该儿子子树的泛化物品与之前子树的泛化物品之和相加，再加上父节点的单个节点的泛化物品就是父节点与其子树所代表的泛化物品之和。
...
【7】树形背包问题中的泛化物品，http://wulala.logdown.com/posts/211367-tree-dependent-generalization-of-the-knapsack-problem
